...
fn get_shortest_way(a: char, b: char, depth: i64, max_depth: i64) -> i64 {
    ...
    for w in ways {
        ...
        for i in 0..way.len() - 1 {
            total += get_shortest_way(way[i], way[i + 1], depth - 1, max_depth);
        }
...
pub fn part1(input: &Vec<String>) -> i64 {
    ...
    for (n, code) in codes.iter().enumerate() {
        ...
        for i in 0..code_a.len() - 1 {
            code_total += get_shortest_way(code_a[i], code_a[i + 1], 2, 2);
        }
